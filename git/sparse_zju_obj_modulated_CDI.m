function sparse_zju_obj_modulated_CDI
...
x_ori=[2*pi/40,pi/5,2];
obj=make_obj(x_ori);

x_num=3;

tic
x=ga(@least_func,x_num,[],[],[],[],zeros(1,x_num),[2*pi,2*pi,1,2*pi]);
toc
...
figure,
subplot(3,3,1),imshow(angle(obj(obj_range,obj_range)),[]),title('object')
subplot(3,3,2),imshow(angle(obj_out(obj_range,obj_range)),[]),title('restored object')
subplot(3,3,4),imshow(I1,[]),title('poly image')
subplot(3,3,5),imshow(I_out,[]),title('restored image')
subplot(3,3,7),hold on,plot(l0),plot(l1),legend('object','restored object')
subplot(3,3,8),bar([x_ori;x]'),legend('original x','restored x'),ylim([0,2*pi])
colormap("parula")
...
disp('end')
    ...
    function y=least_func(x)
        ...
        I_temp=I_temp./max(I_temp(:));
%         I_temp=DR_img(I_temp);
